...
\begin{table}[]
\caption{Keywords of an input dictionary for a standard simulation, see also Fig.~\ref{flow}. Names have been expanded from their original for explanatory power, where questions represent keywords corresponding to booleans. Most of these keywords are actually predeced by an index as this main dictionary is converted into several subdictionaries. The second part of the table \\}
\label{my-label}
\begin{tabular}{ll}
\hline
input fits folder path&antenna table path\\
station info path&integration time\\
...
\caption{The basic class and module structure of the {\sc meqsilhouette} implementation. Solid lines represent a class while dashed lines represent a module. The SimCoordinator class is initialised in the driver script. \label{flow}%
}
\end{figure*}
