...
To simulate pointing errors, we use the implementation built into the {\sc MeqTrees} ``turbo-sim'' task. This functionality includes the capability to convolve station primary beams with the sky model. This is implemented as a an E-Jones matrix.



The beam models available through this function are sinc, Gaussian and the analytic WSRT beam model, however there is not much difference between the different beam models up until the first null. The standard beam model which we will make use is the analytic WSRT beam model \citep{Popping_2008} 
...
\subsection{Jones matrix summary??}

\section{A public online interface}

Table~\ref{tab:parameters} shows the set of parameters needed to run a standard {\sc meqsilhouette} simulation. This moderate number of parameters ($24+6N_{\rm stations}$) can be quickly chosen or selected from a list, especially if most of the defaults are preset and unlikely to change. This speaks to the possibility of a GUI interface which would provide the user with the capability to run a standard simulations without having to delve into code. The capability to run such a simulation would be useful to both theorists and observers in the broader SMBH/mm-VLBI community. For this reason we experimented with such an interface at the Leiden 2015 mm_VLBI workshop\footnote{http://www.astron.nl/other/workshop/mm-VLBI2015}, where we used a early version of the simulator. This was well received by the community present, however we have yet to convert the latest version of the pipeline, also waiting on status.

%briefly the implementation used What was 
