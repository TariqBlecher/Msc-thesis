...
There is another class of pointing error which we did not include in our simulation, but which could be folded in through a combination of the tropospheric and antenna pointing modules. This is the so-called `anomalous pointing error' due to the time-variable phase-gradient across the aperture \citep[e.g.][]{Carilli_1999}. These are pointing errors which will change on the timescale $d_{\rm dish}/v \sim 1-50$~s for $10<d_{\rm dish}<50$~m and $1<v<10$~m/s, where $d_{\rm dish}$ is the diameter of the aperture and hence could be difficult to calibrate.


\citet{holdaway_1998} derives the standard deviation of the pointing error as a fraction of beam width,
\begin{equation}
 \sigma_{\rm pe}(d_{\rm dish}),\theta) = \frac{\sqrt{2 D(d_{\rm dish})}}{\sqrt{\sin\theta}\lambda},
\end{equation}
where $D$ is the structure function and $\theta$ is the elevation angle. Hence for LMT $\theta=30^\circ$, this becomes 
$\sigma_{\rm pe}(50,30^\circ) = ( $
...
\section{Suggested applications and improvements}
