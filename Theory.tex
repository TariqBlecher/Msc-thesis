...
The phenomenology of variability associated with Sgr~A* is complicated due to the plethora of possible variable emission mechanisms. We will discuss several possible physical causes of variability which demonstrate the challenge which variability brings to data analysis and interpretation. Following this, we outline the role of {\sc meqsilhouette} in this scenario.
...
%polarisation
A recent mm-VLBI result is that variability in the polarisation domain is far more rapid than the total intensity \citep{Johnson_2015b}, indicating that the magnetic fields structure is highly dynamic, see section~\ref{sec:unsolved_qs}.  
...
%calibration
As stated eariler, the standard mm-VLBI calibration techniques like fringe-fitting assume that the source is static over the course of the observation. However, if this is not the case, these techniques should be tested to check the consequences of using them under this broken assumption. {\sc meqsilhouette} could help characterise the errors which emerge when a variable source is fringe-fit/self-calibrated in the presence of troposheric-induced errors. These effects could limit image fidelity and DR, but could also cause systematic effects relevant to the EHT measurement objectives (e.g. shadow asymmetry).
