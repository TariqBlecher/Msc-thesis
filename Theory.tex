\chapter{Theory}
%plan st 1
In this chapter we review and develop the theory required to model signal transmission from cosmic source to data product. The first half of this chapter is a broad section centred around radio interferometric concepts while the second half is focused specificially on describing several signal corruptions. 

\section{Radio Interferometry}
%PR 2 St 3

%plan st 1
We introduce radio interferometric theory using the RIME formalism, which serves as a guiding framework for the construction of the {\sc MeqSilhouette} simulator. Following this, we review the technique of self-calibration which is important as it's effects are strongly linked with the signal corruptions, particularly those associated with the troposphere. We then discuss typical mm-VLBI data products and the consequences of breaking the static source assumption.

\subsection{Measurement Equation}\label{sec:RIME}
%PR 2 St 3

%RIME intro and purpose
The Radio Interferometric Measurement Equation (RIME) provides the notation and formalism to model the signal transmission path as a sequence of linear operations. Specificially, it takes into account polarisation, correlation and the correct time-ordering of signal transmission path in an intuitive way. This calculus will also enable a more intuitive view on the phrasing of the relation between calibration and signal corruptions. Here we offer a short derivation and explanation of the RIME following \citet{Smirnov_2011a}.
%Visibility from E-field
Consider an quasi-monochromatic complex valued electric field vector $\bm{E}$, which can be decomposed into an arbitrary two dimensional orthogonal basis in the plane perpendicular to the direction of propagation,
...
The choice of basis represents the polarisation state of the field. All linear transformations of the above electric field can be written by a multiplication with a 2 x 2 complex valued matrix known as a \emph{Jones} matrix,
\begin{equation}
\bm{E'} = \bm{J E}.
\end{equation}
For example the conversion of the electric field to a voltage $\bm{v}$ at an antenna can be specified by such a transformation i.e. $\bm{v} \equiv \bm{E'}$ under multiplication with the appropriate Jones matrix. Multiple effects then can be represented by multiplication of various Jones matricies.
\begin{equation}
\bm{E'} = \bm{J}_n \ldots \bm{J}_1\ \bm{E}
\end{equation}
An interferometer measures the correlation of the voltages from two antennae. This quantity is known as the \emph{visibility}, 
\begin{equation}
\bm{V}_{pq} = \langle {\bm v}_p  {\bm v}_q^H \rangle,
\end{equation}
where $p$, $q$ are refer to the two antennae. Hence the visibility can be related to the electric fields by  Key to the formulation of the RIME is that it is equivalent to the Stokes polarisation formulation, for example in an XY basis,
...
%Connection to polarisation
Note that to properly take into smearing and decoherence, the Jones matricies should also be integrated over the frequency and time averaging interval. Also this description is for a noise-free measurement. Noise can be included by additive contribution to the visibility.
...
\end{equation}
where $l$, $m$, $n = \sqrt{1-l^2-m^2}$ are the direction cosines and $\lambda$ is the signal wavelength.
...
The rules of commutivity of matricies allows us flexibility in the model and correction of signal corruptions, in particular, we know that scalar matrices commute with everything; Diagonal matrices commute among themselves and Rotation matrices commute among themselves.

G-Jones, E-jones

Direction independent Jones terms, for two independent linear dipoles. 
\begin{equation}
\bm{G} =
\left(
\begin{array}{cc}
...
%Why the primer
Self-calibration is a primary method for the estimation of station gains as a function of time and frequency. The interaction of this calibration procedure with variable sources and tropospheric and instrumental signal corruption is of interest. A basic Self-cal will always be applied, typically averaging over 10~s, approximately the coherence time of the atmosphere.

%Self-calibration from point of view of RIME
From the point of view of the RIME, estimate the station based G-Jones terms.
