...
\begin{itemize}
 \item 1. assume an initial sky model (typically a point source if observing a known bright calibrator),
 \item 2. solve for station gains (but not their derivatives) in equation~\ref{eq:G_rime},
 \item 3. image and deconvolve (i.e. the effect of the synthesised beam is partially removed)
 \item 4. run source finder -> update sky model,
 \item 5. repeat steps 2 and 4 until a specified flux threshold is reached.
\end{itemize}
...
%%%%THE BELOW DISCUSSION IS NOT PUT INTO ANY GOOD CONTEXT
In the literature, the uncertainty on the closure phase is calculated in various ways. One method which is typically used for simulations and predictions is model dependent  and is given as a function of the SNR $s$ of each baseline \citep{Rogers_1995},
...
%ISCO - definition and link


%Phenomenology

%relation between frequency and size of emission as introduced in the introduction (i.e the $\tau \sim 1$ surface)
...
Recent EHT and space-VLBI observations have shifted focus away from the well-studied Gaussian convolution effect of ISM scattering and onto the presence of stochastic scattering-induced substructure. To understand this phenomenon, we must first develop the theory to be sensitivity to the averaging effects of the observation. 
...
\end{equation}
where each component of $N_{pq}$ is sampled from a complex-valued Gaussian distribution, $|N_{pq}| \sim (0, \Delta S_{\rm ij}^2)$.
